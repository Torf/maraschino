...
def speedtest(ip=None, port=None, webroot=None, script_id=None):
    ...
    update_status('Testing from %(isp)s (%(ip)s)...' % config['client'], ip, port, webroot, script_id)
    ...
    print_()
    print_('Download: %0.2f M%s/s' % ((dlspeed / 1000 / 1000) * 8, 'b'))
...
def main(argv):
    ...
    speedtest(ip, port, webroot, script_id)
